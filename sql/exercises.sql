...
SELECT * FROM cliente
LEFT JOIN cliente_pf ON (cliente.id = cliente_pf.id)
LEFT JOIN cliente_pj ON (cliente.id = cliente_pj.id)

SELECT * FROM pedido
LEFT JOIN item_pedido ON (pedido.id = item_pedido.id_pedido)
LEFT JOIN produto ON (item_pedido.codigo_barra = produto.codigo_barra)
...
produto.nome,
produto.valor_venda,
item_pedido.sequencial,
item_pedido.valor_unitario,
(item_pedido.quantidade * item_pedido.valor_unitario) AS valor_total
FROM pedido
LEFT JOIN item_pedido ON (pedido.id = item_pedido.id_pedido)
LEFT JOIN produto ON (item_pedido.codigo_barra = produto.codigo_barra)
WHERE id_cliente = 1